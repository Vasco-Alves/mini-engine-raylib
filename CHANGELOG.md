...
- **Scene API in Lua**: `Scene.find`/`Scene.find_all` (entity lookup by name), `Scene.create` (empty entity), `Scene.spawn` (clone an existing entity — prefab-style, with fresh script instances, its own asset references, and a live physics body when the simulation is running), and `Scene.load` (**runtime scene switching** — menus and level transitions in exported games). Scene switches are deferred to the end of the frame and rebuild the physics world; in the editor they're treated as runtime transitions, so stopping play still restores the scene you were editing (and Ctrl+S keeps pointing at it).
- **Engine API in Lua**: `Engine.quit()` (closes an exported game; stops play mode in the editor), `Engine.time()`, `Engine.warn`/`Engine.error`.
- **Vector3 math in Lua**: full operators (`+ - * / ==`, unary minus, `tostring`) and methods (`length`, `length_sq`, `normalized`, `dot`, `cross`, `distance`, `lerp`), plus a `Color` type — and the `table` standard library is now open to scripts.
- **Component access from Lua**: Material, point/directional Light and Camera join the Transform (`get_material`, `get_light`, `get_directional_light`, `get_camera` — live references, `nil` when absent), plus `get_name`/`set_name`/`get_id`.
- **Physics from Lua**: `get_velocity`, `apply_impulse`, `set_angular_velocity`, and `teleport` (moves the transform *and* the Jolt body). Engine-side `physics::add_body`/`remove_body` create/remove single bodies mid-play, which is what makes spawned entities physical and destroyed entities collision-free.
- **Music from Lua**: `play_music`/`stop_music`/`pause_music`/`resume_music`.
- **First-person camera support in Lua**: `entity:set_look(yaw, pitch)` aims the entity's Camera from yaw/pitch (degrees, pitch auto-clamped) — a player with a Camera becomes an FPS camera in one call per frame. Paired with `Input.lock_cursor()`/`Input.unlock_cursor()` for mouse-look (the cursor hides and recenters so the `LookX`/`LookY` axes keep flowing); the editor force-unlocks the cursor on Stop so it can never get stranded.
- **Cameras can be child entities**: the camera eye now uses the entity's *world* position (via `TransformComponent::world_position()`) across the rasterizer, the path tracer, `set_look`, and the editor's camera/light gizmos, so a Camera parented to a player at eye height follows the body correctly. Previously the camera always used its local position, so a parented camera rendered (and its gizmo drew) near the origin — the recommended FPS rig (Player body + child PlayerCamera at head height) now works.
- **Per-axis rotation constraints on RigidBody**: **Freeze Rotation X / Y / Z** checkboxes (inspector + serialized), applied through Jolt's `mAllowedDOFs`. Freeze X and Z to keep an upright character from tipping over while it walks — the missing piece for a physics-driven FPS player without a capsule controller.
- **Collision callbacks**: scripts can define `on_collision_enter(self, other)` and `on_collision_exit(self, other)`. A Jolt ContactListener buffers contacts across the worker threads, deduplicates them per entity pair, and the events dispatch right after the physics step (scripts → physics → collision callbacks → transforms). The demo's jump script now does a real grounded check with a contact counter. Body sleeping is disabled: Jolt reports contact removal when an island falls asleep, which would have fired a false exit on every body at rest for half a second.
- **Trigger volumes**: a RigidBody can be marked **Is Trigger** (inspector checkbox, serialized) — overlaps fire the collision callbacks but produce no physical response. Pickups, damage zones, level exits.
- **Raycasts**: `Physics.raycast(origin, direction, max_distance)` in Lua returns the closest hit (`entity`, `point`, `normal`, `distance`) or `nil`; engine-side `me::physics::raycast` underneath.
- **Real-time shadow maps**: the forward renderer (Edit/Play viewport and exported games) now draws directional-light shadows — a depth-only pass from the sun's view into a depth texture, orthographic volume following the camera with texel snapping (no edge shimmer), sampled with 3×3 PCF and a slope-scaled bias. The depth pass reuses the exact geometry path of the main pass. Each DirectionalLight gets **Cast Shadows**, **Shadow Extent** (world-unit area the map covers — sharpness vs reach) and **Shadow Resolution** (512–4096) controls in the inspector, all serialized per light; the path tracer is untouched (it always computed real shadows — Play mode just stops looking flat next to it).
- **RT Play — real-time path-traced play mode**: an "RT" toggle next to the toolbar's RENDER button makes Play render through the path tracer (from the scene's camera, one burst of samples per frame, scene rebuilt every frame). Designed for the retro/pixelated use case: a low Resolution Scale buys the per-frame samples, and a **Lock Noise Pattern** option freezes the residual noise into a stable dither instead of animated static. Samples/frame is tunable live in Raytracer Settings.
- **Raytraced game exports**: a "Raytraced renderer" checkbox in File > Export Game ships the game with the path tracer as its renderer — `game_config.json` carries a snapshot of the Raytracer Settings (resolution scale, samples/frame, bounces, sky, exposure), and the runtime renders at the low internal resolution and upscales with nearest filtering (crisp pixels). Needs GPU compute (OpenGL 4.3); falls back to the CPU tracer where unavailable.
- **Raytracer feature toggles + quality presets**: a Render Features section in Raytracer Settings with four switches — Indirect Lighting (GI), Soft Shadows, Reflections, Refraction — and one-click **Full / Lite / Flat** presets, all feature-matched across the CPU and GPU backends and carried into raytraced game exports. The point is control over the noise/speed tradeoff: every noise source in a path tracer is a *random* sample, so turning off the stochastic features (GI, soft shadows) yields a **noise-free image at one sample per pixel** that runs at full framerate — while mirror reflections and sharp glass stay on, because they're deterministic. The panel shows a live "Noise-free" indicator when the current combo converges instantly. Flat (direct light + hard shadows only) is the ideal renderer for a simple retro game; Lite keeps the ray-traced look (mirrors, glass) without the grain; Full is reference quality.
- **Camera gizmos**: cameras now draw an editor gizmo in Edit mode (wireframe body + FOV-accurate view frustum toward the look-at target, yellow = the active camera) and are click-selectable in the viewport, like lights.
- New headless test suites registered with CTest: `lua_api` (Vector3/Color math, sandboxing, Entity bindings, no-op physics safety) and `physics_events` (contact enter/exit with pair-dedup and net-balance invariants, sensors overlapping without colliding, raycast hits/misses, exit events when a body is removed mid-play including the asleep-body case). The `shader_check` dev tool now validates the lighting vertex/fragment pair as well as the raytracer compute shader.
- **Docs**: a full Lua scripting reference at `docs/LUA_API.md`.

### Changed

- **Physics stepping is hardened against slow frames**: a frame is subdivided into collision steps of at most ~1/60 s (Jolt's recommendation) and clamped after long stalls, so a laggy machine can no longer tunnel fast bodies through thin colliders or make the simulation explode catching up.
- **The game owns the keyboard during Play**: editor shortcuts (gizmo switches Q/W/R/S, Delete/Ctrl+D, save, undo…) are suppressed while playing — they used to fire on the same keys a game reads (WASD overlapped the gizmo switches) and could edit the scene mid-playtest. Only Ctrl+P (stop) stays live. Exported games never had editor shortcuts.
- **Playtest focus**: Play now starts unfocused — the game gets no input and the cursor stays free until you **click the viewport**, and **Escape** hands the mouse back to the editor. This separates the game's own cursor intent (`Input.lock_cursor`/`unlock_cursor`, what a shipped game uses) from the editor's playtest focus (which overrides it while unfocused), so you can tweak Raytracer Settings or panels mid-playtest without the camera following the mouse. Backed by a new `me::input::set_input_gate` that neutralizes all input queries and defers cursor-lock while closed. The editor's own tools are also dormant during Play now — the RMB fly-camera (which used to grab and then release the cursor, unlocking it on a click) and viewport gizmo/click-picking only run in Edit mode, so playtest clicks go to the game, not the editor.

### Performance

- **GPU tracer uniform locations are cached once at shader load** instead of being re-queried (a `glGetUniformLocation` per uniform) every frame — that was ~25 wasted GL round-trips per frame in RT Play and the offline render.
- **RT Play skips the BVH + GPU-buffer rebuild when the scene didn't change.** Each frame hashes the raw bytes of every component the flattener/BVH read (transforms, shapes, models, materials, lights); a camera-only move or a paused game now reuses last frame's structures instead of rebuilding and re-uploading everything. The hash is correct by construction — any edit, or any add/remove (the sparse-set reorders), changes it.
- **SSBO uploads update in place** (`rlUpdateShaderBuffer`) and only reallocate VRAM when a buffer must grow, instead of freeing and re-allocating all seven buffers every frame — so scenes that genuinely change each frame (a moving player) still avoid the per-frame alloc churn.

### Fixed

- **First-person view no longer "bumps" when you stop moving**: `set_look` runs before physics moves the body, so it aimed the camera from a one-frame-stale eye while the renderer used the current one — with a near look-target that tilted the view a few degrees while moving and snapped straight on stop. The target is now placed far down the view ray, making the sub-frame eye shift angularly negligible.
- **Raytraced exports guard against stale runtimes**: exporting copies the prebuilt `game` runtime sitting next to the editor — if that binary predates the raytraced renderer, the shipped game silently fell back to raster. The runtime now embeds a capability marker (`me-runtime-cap:raytraced;`, logged at boot), and the export refuses a raytraced package when the runtime lacks it, telling you to rebuild the `game` target. Exported games also echo engine logs to stdout now — load errors used to vanish (only the editor's console panel subscribed to them).
- **Console log spam**: the raytracer logged "GPU Buffers Flattened" and "SSBOs Uploaded" on every accumulation reset — which RT Play turns into an every-frame loop. Flatten stats now log only when the scene's shape actually changes, and the redundant upload line is gone. Same pass for the other frame-rate offenders: a script whose `update` keeps erroring logs the error **once** (suppressed until the file is edited — hot-reload re-arms it) instead of flooding at 60 Hz, and `get_transform()` on a destroyed entity just returns nil (the documented contract) instead of logging an error per call.
- **Exported games no longer draw the editor grid**: `render_world` drew it unconditionally; the grid is now drawn by the editor itself, in Edit mode only.
- **Cloned entities no longer share native handles**: duplicating (editor) or spawning (Lua) an entity with a Model/AudioSource/BackgroundMusic now acquires its own ref-counted asset reference — a raw handle copy meant the asset was released twice when both entities died, unloading it while still in use.
- **Cloning a RigidBody mid-play no longer shares the source's Jolt body** (two entities fought over one body); the clone gets its own.
- **Destroying an entity (or removing its RigidBody) during play removes its Jolt body** — previously the invisible body stayed behind and kept colliding.
- **Scene loads release the outgoing scene's asset handles** — switching scenes used to pin every model/sound of the old scene in memory for the rest of the session.

## [1.0.0] - 2026-06-12

The first stable release. The identity is complete and shipping: *build a scene once — press Play to run it as a game, or press Render to path-trace it into stills and animations.* Both halves produce standalone artifacts (exported games, rendered image sequences), the editor is undo-safe end to end, and the whole thing packages into a redistributable zip.

### Added

- **Demo project**: new projects are seeded with a showcase scene — glass and mirror spheres, a gold dragon model, an emissive cube, a roughness lineup, and a physics cube with a Lua jump script + sound (press Play, hit SPACE). A sample animation sidecar (camera dolly + emission ramp) is included, ready for Render Animation. `Ctrl+N` still creates a minimal clean scene.
- **App icon**: the editor and game executables carry the engine icon (play triangle + render orb), and the editor sets it as the window/taskbar icon at runtime.
- **Help menu**: a Controls cheat-sheet window (every shortcut) and the About dialog.
- **Cross-platform groundwork**: macOS caps OpenGL at 4.1, so the build now selects the GL version per platform (the GPU path-tracer backend is unavailable on Apple platforms; the raytracer detects this at runtime and the CPU backend keeps working). AVX2 is only requested on x86-64, unblocking ARM builds.

### Changed

- **The editor ships as `MiniEngine.exe`** (the product name) instead of `editor.exe`.
- A missing game runtime at export now explains the fix for packaged installs, not just source builds.

### Fixed

- **Transform hierarchy cycle guard**: a corrupt or hand-edited scene file containing a parent cycle no longer overflows the stack — the loop is broken with a warning, cycle "islands" still update standalone, and entities whose parent id doesn't resolve are treated as roots instead of freezing forever.
- **Scene loading resilience**: a malformed component is logged and skipped instead of aborting the whole scene load.
- **The last non-undoable edits**: hierarchy drag-drop reparenting (including Unparent) and Lua script attach/detach now go through the command history.
- New headless `animation_eval` test suite pins down keyframe interpolation, easing, clamping and JSON-lerp semantics (registered with CTest).

## [0.17.0] - 2026-06-11

The release-readiness update: the engine's second front-end (the game runtime) ships, and the project gains the release hygiene expected of a 1.0 candidate.

### Added

- **Game runtime (`game/`)**: a standalone `me::Application` front-end with no editor and no ImGui. It reads `game_config.json`, mounts the packaged assets, loads the boot scene and runs it in play mode — physics, Lua scripts and 3D audio — rendered from the scene's active `CameraComponent` (with a safe fallback view when a scene has no camera).
- **File > Export Game…**: packages a standalone game Godot-style — the prebuilt runtime is copied and renamed, engine shaders and the project's assets are copied next to it, and a `game_config.json` records window settings and the boot scene. Native folder picker, boot-scene selector, no compilation at export time.
- **LICENSE**: the project is now formally MIT licensed.
- **Versioning**: single-source version constant (`core/version.hpp`), a Help > About dialog in the editor, and this changelog brought back into service.
- **Scene format version field**: scene files now carry `"version"`, so future format changes can be migrated (or at least warned about) instead of silently misread.
- **Packaging**: CMake install rules + CPack produce a redistributable zip (editor + game runtime + assets + docs) via `cpack -G ZIP`.

## [0.16.0] - 2026-06-11

The identity update: the engine now states — and shows — what it is: *build a scene once; press Play to run it as a game, or press Render to path-trace it into film.*

### Added

- **Per-mode workspaces**: Edit and Render modes each remember their own panel set (View > Panels) and dock layout, swapping automatically on mode change. Render mode drops the content browser/console and brings in the animation timeline and raytracer settings.
- **Mode visual identity**: color-coded viewport frame (green = playing, purple = rendering), a toolbar mode badge, and the mode in the window title.
- **Shipped default layouts**: fresh installs start with the layouts under `editor/assets/layouts/` (exported via Layout > Save As Shipped Default), so the first run looks like the author's setup.
- **Explicit-save layout model**: ImGui's continuous ini autosave is disabled; layouts persist only when saved (Layout > Save Layout), with in-session memory across mode switches and a Reset to Default.

### Changed

- **Asset ownership**: the runtime shaders (lighting + raytracer compute) moved from `editor/assets/` to `engine/assets/` — they were always loaded through the `engine://` mount and belong to the engine, not the editor.
- The editor's offline-render flows (PNG export + animation render) moved into their own translation unit (`editor_offline_render.cpp`).
- README/ARCHITECTURE rewritten around the one-scene-many-consumers identity.

## [0.15.0] - 2026-06-11

The animation update: keyframe the camera, the environment and entity components, then render the result as an image sequence.

### Added

- **Keyframe animation**: camera track (position, look-at, FOV, exposure, aperture, focus distance), environment track (sky horizon/zenith colors, sky intensity, ambient — sunsets are two keys), and entity tracks that animate whole components (Transform, Material, shapes, lights) through the component registry — captured as JSON snapshots with every numeric field interpolated, zero per-component animation code.
- **Timeline panel**: a real timeline with a time ruler, scrubbing playhead, draggable keyframe diamonds, per-key easing (linear/smooth), right-click context menus, double-click to jump, Ctrl+scroll zoom with pinned track labels, and direct value editing for camera/environment keys. All keyframe edits are undoable.
- **Animation preview**: plays in the Edit viewport in real time, and inside the path tracer in Render mode (one progressive sample per frame).
- **Offline animation render**: renders frame-by-frame at export resolution/samples into `renders/anim_<timestamp>/frame_0001.png …` with per-frame and per-sample progress, cancel support, and the ffmpeg one-liner logged for joining into a video.
- **Animation persistence**: tracks save to a `<scene>.anim.json` sidecar with the scene; entity tracks re-bind by entity name across scene loads.

## [0.14.0] - 2026-06-10

The editor quality-of-life update: undo for everything, real scene management, and an editor that no longer burns the GPU while idle.

### Added

- **Structural undo/redo**: adding/removing components, creating/deleting/duplicating entities — all undoable. Deleting an entity snapshots every component plus hierarchy links; undo restores it and re-claims its children.
- **Scene management**: File > Open Scene, Save Scene As (Ctrl+Shift+S), an unsaved-changes guard on Exit/New/Open, and a dirty marker (`*`) in the title bar.
- **Hierarchy polish**: inline rename (F2 / double-click), Duplicate and Rename in the context menu, and an Edit menu with Undo/Redo/Duplicate/Delete.
- **Inspector precision**: hold Alt for 10x finer drag control on every field; gizmo snapping with Ctrl was joined by camera speed modifiers (Shift = sprint, Ctrl = precision) and scroll-to-tune fly speed with an on-screen toast.
- **Stats overlay** (View > Stats Overlay): FPS/frame time, entity count, and raytracer resolution/backend/sample progress in Render mode.

### Changed

- **Frame pacing**: the editor was running uncapped; Edit/Play now cap to the monitor refresh rate and Render mode uncaps (every frame is a path-tracer sample).
- Scene loads route through one guarded path; opening a scene from the content browser previously left Ctrl+S pointed at the *old* scene file (a data-loss bug) and kept a stale undo history that could resurrect entities from the previous scene.
- Component removal now releases native handles (models/audio) through the component registry, fixing leaks when removing asset-backed components.
...
- **Soft shadows**: point lights gained a radius and directional lights an angular size; shadow rays sample the light's disk/cone, so shadows soften with distance.
- **Physical glass**: Fresnel (Schlick) reflectance at interfaces and Beer–Lambert absorption through the interior — tinted glass darkens with thickness and its cast light takes the glass color. A per-material **Tint Strength** scales the absorption density (0 = always clear). Inverse-square falloff for point lights.
- **Depth of field**: thin-lens aperture + focus distance with **click-to-focus** in the render view; exposure control; **firefly clamp** to suppress bright noise specks.
- **Camera component panel** in the inspector, and play mode now renders from the scene's active camera instead of the editor fly-cam.
- **Export estimates**: the export dialog shows MP/sample, approximate VRAM and workload warnings, recommending a backend when settings are heavy.

### Fixed

- **GPU driver resets on heavy exports (TDR)**: the compute dispatch is now split into row bands sized against the bounce count, so no single dispatch can outlive the OS GPU watchdog. High-resolution GPU exports no longer crash the app.
- **Analytic cube interior hits**: rays starting inside a cube (refraction) reported no exit hit, so glass cubes never tinted — both backends now fall back to the exit face, matching the sphere's behavior.
- **Depth-of-field lens sampling** was per-frame instead of per-pixel, making the whole image lurch while accumulating instead of blurring.
- **Export correctness**: exports previously rendered at viewport resolution regardless of the dialog settings (the viewport size tracker fought the export resize), and the viewport pass kept rendering during exports. Offline renders now own the raytracer, pause the viewport, restore every overridden setting, and support cancel.

## [0.12.0] - 2026-06-09

The architecture update: one registration per component now drives everything, and the GPU path tracer becomes the default backend.

### Added

- **Component registry** (`me::ecs`): a single per-component registration providing has/save/load/clone/remove/on-destroy operations. Scene serialization, entity duplication, native-handle cleanup — and later undo and animation — all derive from it.
- **GPU compute path tracer** as the default backend, mirroring the CPU feature set (BVH/BLAS traversal, NEE direct lighting, progressive accumulation, ACES, Halton AA), with shared scene flattening into SSBOs.
- **Headless tests**: a scene save→load→save round-trip test (CTest) and a compute-shader compile check used to validate GLSL changes without driving the editor.

### Changed

- Scene save/load rewritten on top of the registry with a three-pass load (create + id map, load components, relink hierarchy).
- One canonical per-frame order (`world_update`): scripts → physics → transforms, removing a one-frame physics lag.
- The inspector was rebuilt around a component table (single source of truth for panels and the Add Component menu) with RAII section headers fixing a latent ImGui tree-stack imbalance.
- OpenGL 4.3 is requested through raylib's own configuration knob instead of a leaky global define.

### Removed

- The dormant 2D component scaffolding (Shape2D/Camera2D/Sprite) — the engine is 3D-focused; 2D can return as ordinary registry entries when wanted.

### Fixed

- `me::audio::update()` was never pumped, so streamed music stalled after a few seconds.
- Entity duplication and deletion now go through the registry (duplicating an entity previously aliased the source's children list; deletion leaked native handles).
