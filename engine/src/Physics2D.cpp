...
namespace {
	...
	// --- Collision Recording ---
	std::vector<me::physics::Collision> s_Collisions;
	...
	inline AABB MakeAabb(const me::components::Transform2D& t, const me::components::AabbCollider& c) {
		float cx = t.x + c.ox;
		float cy = t.y + c.oy;
		return AABB{ cx - c.w * 0.5f, cy - c.h * 0.5f, c.w, c.h };
	}
	...
	inline bool OverlapCircleAabb(const CircleShape& c, const AABB& b, float& outDx, float& outDy) {
		...
		if (dist2 <= 1e-8f) {
			...
			if (right < minSide) { minSide = right; outDx = (c.r + right); outDy = 0.0f; }
			if (top < minSide) { minSide = top; outDx = 0.0f; outDy = -(c.r + top); }
			if (bottom < minSide) { outDx = 0.0f; outDy = (c.r + bottom); }
			return true;
		}
	...
	inline MassInfo ComputeMass(bool hasAabb, const me::components::AabbCollider* aabb,
		...
		if (hasCircle && circle) {
			...
			m = r * r; // rough approximation (area-like)
...
namespace me::physics {
	...
	void Update(float dt) {
		...
		// 2. Gather Static Bodies (Need ID now for events!)
		struct StaticAabb {
			me::EntityId e;
			me::components::Transform2D* t;
		...
		if (auto* aabbPool = reg.TryGetPool<me::components::AabbCollider>()) {
			for (auto& kv : aabbPool->data) {
				...
				auto* t = reg.TryGetComponent<me::components::Transform2D>(e);
				if (t) staticsAabb.push_back({ e, t, &kv.second });
			}
		}
		...
		if (auto* circlePool = reg.TryGetPool<me::components::CircleCollider>()) {
			for (auto& kv : circlePool->data) {
				...
				auto* t = reg.TryGetComponent<me::components::Transform2D>(e);
				if (t) staticsCircle.push_back({ e, t, &kv.second });
			}
		}
		...
		for (std::size_t i = 0; i < n; ++i) {
			for (std::size_t j = i + 1; j < n; ++j) {
				...
				if (A.circle && B.circle) {
					collided = OverlapCircleCircle(MakeCircle(*A.t, *A.circle), MakeCircle(*B.t, *B.circle), dx, dy);
				} else if (A.circle && B.aabb) {
					collided = OverlapCircleAabb(MakeCircle(*A.t, *A.circle), MakeAabb(*B.t, *B.aabb), dx, dy);
				} else if (A.aabb && B.circle) {
					collided = OverlapCircleAabb(MakeCircle(*B.t, *B.circle), MakeAabb(*A.t, *A.aabb), dx, dy);
					if (collided) { dx = -dx; dy = -dy; }
				} else if (A.aabb && B.aabb) {
					collided = OverlapAabbAabb(MakeAabb(*A.t, *A.aabb), MakeAabb(*B.t, *B.aabb), dx, dy);
				}

				if (!collided) continue;

				// Record Collision
				s_Collisions.push_back({ A.e, B.e });
				...
				A.t->x += nx * moveA; A.t->y += ny * moveA;
				B.t->x -= nx * moveB; B.t->y -= ny * moveB;
				...
				// Restitution (bounciness) - hardcoded 0 for now, could be in component
				float restitution = 0.0f;
				float jj = -(1.0f + restitution) * relAlongN / invSum;
				float impulseX = jj * nx;
				float impulseY = jj * ny;

				A.v->vx -= impulseX * invA; A.v->vy -= impulseY * invA;
				B.v->vx += impulseX * invB; B.v->vy += impulseY * invB;
			}
		}
		...
		if (g_boundsEnabled) {
			for (auto& b : bodies) {
				if (b.aabb) {
					...
					bool hit = false;
					if (a.x < g_minX) { nx += (g_minX - a.x); b.v->vx = 0.0f; hit = true; }
					if (a.y < g_minY) { ny += (g_minY - a.y); b.v->vy = 0.0f; hit = true; }
					if (a.x + a.w > g_maxX) { nx -= (a.x + a.w - g_maxX); b.v->vx = 0.0f; hit = true; }
					if (a.y + a.h > g_maxY) { ny -= (a.y + a.h - g_maxY); b.v->vy = 0.0f; hit = true; }
					b.t->x = nx; b.t->y = ny;
					// Optionally record world collision: s_Collisions.push_back({b.e, 0});
				} else if (b.circle) {
					CircleShape c = MakeCircle(*b.t, *b.circle);
					float nx = b.t->x, ny = b.t->y;
					float l = c.cx - c.r, r = c.cx + c.r, t = c.cy - c.r, bot = c.cy + c.r;
					bool hit = false;
					if (l < g_minX) { nx += (g_minX - l); b.v->vx = 0.0f; hit = true; }
					if (t < g_minY) { ny += (g_minY - t); b.v->vy = 0.0f; hit = true; }
					if (r > g_maxX) { nx -= (r - g_maxX); b.v->vx = 0.0f; hit = true; }
					if (bot > g_maxY) { ny -= (bot - g_maxY); b.v->vy = 0.0f; hit = true; }
					b.t->x = nx; b.t->y = ny;
				}
			}
		}
...
//
//	struct AABB { float x, y, w, h; };
//	struct CircleShape { float cx, cy, r; };
//
//	inline AABB MakeAabb(const me::components::Transform2D& t, const me::components::AabbCollider& c) {
...
//	}
//
//	const std::vector<me::physics::Collision>& GetCollisions() {
//		return s_Collisions;
//	}
//
//	void Update(float dt) {
